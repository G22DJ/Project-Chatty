...
const HardwareSensors: React.FC<HardwareSensorsProps> = ({ data, deviceType, themeColor }) => {
  ...
  return (
    <div className={`flex items-center ${isCompact ? 'gap-1.5 px-2 py-1' : 'gap-4 px-4 py-2'} bg-white/5 rounded-2xl border border-white/5 backdrop-blur-md`}>
      <div className="flex items-center gap-1.5 group cursor-help" title={`Platform: ${data.platform}`}>
        <i 
          className={`fas ${deviceType === 'mobile' ? 'fa-mobile-screen' : deviceType === 'wear' ? 'fa-watch' : deviceType === 'tv' ? 'fa-tv' : deviceType === 'auto' ? 'fa-car' : 'fa-desktop'} text-[9px] lg:text-[10px]`}
      ...
      <div className="flex items-center gap-1.5">
        <i className={`fas ${data.online ? 'fa-wifi' : 'fa-plane'} text-[9px] lg:text-[10px] ${data.online ? 'text-emerald-500' : 'text-red-500'}`}></i>
      ...
      {data.battery !== undefined && (
        <>
          <div className="w-[1px] h-3 bg-white/10"></div>
          <div className="flex items-center gap-1.5">
            <i className={`fas ${data.charging ? 'fa-battery-bolt' : 'fa-battery-three-quarters'} text-[9px] lg:text-[10px] ${data.battery < 0.2 ? 'text-red-500' : 'text-emerald-500'}`}></i>
