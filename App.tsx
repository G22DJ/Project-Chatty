...
const TranscriptionList = React.memo(({ transcriptions, deleteTranscription, themePrimary, assistantProfilePic }: { transcriptions: TranscriptionEntry[], deleteTranscription: (id: string) => void, themePrimary: string, assistantProfilePic?: string }) => {
  ...
  return (
    <>
      {(Object.entries(grouped) as [string, TranscriptionEntry[]][]).map(([dateStr, items]) => (
...
const App = () => {
  ...
  const startSession = async (initialText?: string) => {
    try {
      ...
      const memoryPrompt = memories.length > 0 ? `Knowledge Bank: ${memories.map(m => m.fact).join(' | ')}.` : "Knowledge Bank is empty.";
  ...
  const handleSendText = useCallback(() => {
    ...
    setInputText('');
    if (state === AssistantState.IDLE) startSession(text);
    ...
    <div className="flex flex-col h-full w-full transition-all duration-1000 overflow-hidden">
      <header className="m-[var(--ui-gap)] p-4 lg:p-6 rounded-[var(--ui-radius)] flex items-center justify-between z-50 glass animate-fade-blur-in">
        <div className="flex items-center gap-3 lg:gap-6 group">
          <div className="w-10 h-10 lg:w-14 lg:h-14 rounded-[var(--bubble-radius)] overflow-hidden border border-white/10 relative transition-all duration-700 hover:scale-110 shadow-xl group/avatar">
            {prefs.assistantProfilePic ? (
              <img src={prefs.assistantProfilePic} className="w-full h-full object-cover" alt="AI" />
            ) : (
              <div className="w-full h-full bg-blue-500/10 flex items-center justify-center">
                <i className="fas fa-user-astronaut text-white text-lg lg:text-xl"></i>
              </div>
            )}
          </div>
